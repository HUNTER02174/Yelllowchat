...
ADMIN_ID = 6447881580          # آیدی عددی خودتان
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != ADMIN_ID and not await check_membership(user_id, context):
        ...
        await update.message.reply_text(
            "⚠️ برای استفاده از ربات موزی چت، ابتدا باید در کانال ما عضو شوید:",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID and state.get('action') == 'admin_reply':
        ...
        if target_id:
            try:
                await context.bot.send_message(chat_id=target_id, text=f"📩 پاسخ مدیریت:\n\n{user_text}", parse_mode="Markdown")
    ...
    # ۴. پشتیبانی
        ...
        admin_msg_text = (
            ...
            f"🆔 آیدی واقعی (عدد تلگرام): {user_id}\n\n"
            f"📝 متن:\n{user_text}"
        )
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text="🔔 یک پیام جدید در صندوق پیام‌ها دارید!")
...
if __name__ == '__main__':
    ...
    print("ربات یلو چت (نسخه کامل) روشن شد...")
